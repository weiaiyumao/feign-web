...
public class UserFeignServiceHiHystric implements UserFeignService {
	...
	public BackResult<CreUserDomain> findbyMobile(String mobile) {
		return new BackResult<>(ResultCode.RESULT_FAILED, "user-provider-service服务findbyMobile出现异常");
	}

	@Override
	public BackResult<CreUserDomain> findOrsaveUser(CreUserDomain creUserDomain) {
		return new BackResult<>(ResultCode.RESULT_FAILED, "user-provider-service服务findOrsaveUser出现异常");
	}

	@Override
	public BackResult<CreUserDomain> updateCreUser(CreUserDomain creUserDomain) {
		return new BackResult<>(ResultCode.RESULT_FAILED, "user-provider-service服务updateCreUser出现异常");
	}
	...
	public BackResult<CreUserDomain> activateUser(CreUserDomain creUserDomain) {
		return new BackResult<>(ResultCode.RESULT_FAILED, "user-provider-service服务activateUser出现异常");
	}

	@Override
	public BackResult<CreUserDomain> activateUserZzt(CreUserDomain creUserDomain) {
		return new BackResult<>(ResultCode.RESULT_FAILED, "user-provider-service服务activateUserZzt出现异常");
	}
